...
const DashboardLayout: React.FC = () => {
  ...
  const getNavClass = ({ isActive }: { isActive: boolean }) =>
    `flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-semibold transition-[background-color,color,shadow,border-color] duration-200 w-full outline-none focus-visible:ring-2 focus-visible:ring-purple-400/50 dark:focus-visible:ring-mint-400/50 ${isActive
      ? 'bg-white/40 dark:bg-white/10 dark:text-mint-300 text-purple-700 shadow-sm border border-white/50 dark:border-white/10'
      : 'text-slate-600 dark:text-slate-300 hover:bg-white/20 dark:hover:bg-white/5 hover:text-purple-600 dark:hover:text-mint-200 border border-transparent'
    }`;
