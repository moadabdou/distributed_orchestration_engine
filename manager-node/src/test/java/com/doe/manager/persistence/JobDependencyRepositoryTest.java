...
class JobDependencyRepositoryTest {
    ...
    void shouldCascadeDeletesForWorkflow() {
        ...
        JobEntity job1 = new JobEntity(UUID.randomUUID(), JobStatus.PENDING, "{}", 60000L, "l1", now, now);
        job1.setWorkflow(workflow);
        JobEntity job2 = new JobEntity(UUID.randomUUID(), JobStatus.PENDING, "{}", 60000L, "l2", now, now);
    ...
    void shouldFindAndDeleteByDependentJobId() {
        ...
        JobEntity job2 = new JobEntity(UUID.randomUUID(), JobStatus.PENDING, "{}", 60000L, "l2", now, now);
        job2.setWorkflow(workflow);
        JobEntity job3 = new JobEntity(UUID.randomUUID(), JobStatus.PENDING, "{}", 60000L, "l3", now, now);
