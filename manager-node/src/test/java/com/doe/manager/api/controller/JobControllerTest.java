...
public class JobControllerTest {
    ...
    void submitJob_ReturnsCreatedJob() throws Exception {
        ...
        JobResponse mockResponse = new JobResponse(
                jobId, JobStatus.PENDING, "{\"cmd\":\"echo hello\"}", null, null, 0, Instant.now(), Instant.now()
        );
    ...
    void listJobs_ReturnsPaginatedJobs() throws Exception {
        UUID jobId = UUID.randomUUID();
        JobResponse mockResponse = new JobResponse(
                jobId, JobStatus.PENDING, "{}", null, null, 0, Instant.now(), Instant.now()
        );
    ...
    void getJob_ReturnsJobDetails() throws Exception {
        UUID jobId = UUID.randomUUID();
        JobResponse mockResponse = new JobResponse(
                jobId, JobStatus.RUNNING, "{}", "Success", UUID.randomUUID(), 0, Instant.now(), Instant.now()
        );
